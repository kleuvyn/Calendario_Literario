...
const MonthCalendar = dynamic(
  () => import("@/components/month-calendar").then(mod => mod.MonthCalendar),
  {
    ssr: false,
    loading: () => <div className="p-10 text-center opacity-70">Carregando calendário...</div>,
  }
)

const MonthReview = dynamic(
  () => import("@/components/month-review").then(mod => mod.MonthReview),
  {
    ssr: false,
    loading: () => <div className="p-10 text-center opacity-70">Carregando resumo...</div>,
  }
)

const UserProfileEdit = dynamic(
  () => import("@/components/user-profile-edit").then(mod => mod.UserProfileEdit),
  {
    ssr: false,
    loading: () => null,
  }
...
export interface HomeClientSummary {
  userGoal: number;
...
export function HomeClient({ initialSummary, currentYear, initialSession }: HomeClientProps) {
  ...
  useEffect(() => {
    ...
    if (savedGoals) {
      try { JSON.parse(savedGoals) } catch { /* ignore */ }
    }
  ...
  useEffect(() => {
    if (!session?.user?.email || summary) return
    let active = true
    setIsSummaryLoading(true)

    getReadingSummary(session.user.email, currentYear)
      .then((data) => {
        if (active) {
          setSummary(data)
        }
      ...
      .catch((error) => {
        ...
        if (active) setSummary({ userGoal: 12, totalReadThisYear: 0, readingNowThisYear: 0, plannedThisYear: 0 })
      })
      .finally(() => {
  ...
  const getFirstName = () => {
    if (session?.user?.name) {
      const name = session.user.name.toString().trim()
      return name.charAt(0).toUpperCase() + name.slice(1).split(' ')[0].toLowerCase()
    }
    return 'Usuário'
  }
  ...
  const myBooksGoal = summary?.userGoal ?? 12
  const totalReadThisYear = summary?.totalReadThisYear ?? 0
      ...
      <main className="grow p-3 sm:p-4 md:p-10">
        <div className="mx-auto max-w-5xl">
          <motion.div 
            ...
            className="mb-8 sm:mb-10 flex flex-col lg:flex-row items-start sm:items-center justify-between p-4 sm:p-6 md:p-8 rounded-3xl sm:rounded-4xl border border-black/5 relative overflow-hidden shadow-sm gap-4"
                ...
                <div className="flex flex-col sm:flex-row sm:flex-wrap items-start sm:items-end justify-between gap-3 sm:gap-4">
                  <div>
                    ...
                    <h2 className="text-3xl sm:text-5xl md:text-6xl font-black italic tracking-tighter" style={{ color: theme.primary }}>
                      {isSummaryLoading ? '...' : `${totalReadThisYear} / ${myBooksGoal}`}
                    </h2>
